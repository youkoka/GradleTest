...
import com.mitake.build.object.UtilityObject;
...
public class ServiceController extends BaseController{
	...
	@Value("${mitakeServiceUrl}")
	private String mitakeServiceUrl;
	...
	public ResponseEntity<Void> buildService(
        ...
        try {
        	...
        	//! device 
        	String device = "";
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getDevice())) {
        	...
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getProjectFolder()) && !UtilityObject.isStrEmpty(buildJsonObj.getProjectName())) {
        	...
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getProjectScheme())) {
        	...
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getProjectTarget())) {
        	...
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getProjectVersion())) {
        	...
        	//! desc
        	String desc = "";
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getDesc())) {
        		
        		desc = buildJsonObj.getDesc();
        	}
        	
        	String bundleId = "";
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getBundleID())) {
        		
        		bundleId = buildJsonObj.getBundleID();
        	}
        	
        	String bundleName = "";
        	if(!UtilityObject.isStrEmpty(buildJsonObj.getBundleName())) {
        	...
        	if(buildJsonObj.getSvn() != null) {
	    		...
	    		for(int i = 0; i < buildJsonObj.getSvn().length; i++) {
	    			...
	    			if(!UtilityObject.isStrEmpty(buildSvn.getLink()) && !UtilityObject.isStrEmpty(buildSvn.getDest())) {
    		...
    		if(device != null) {
    			...
    			if(device.equals("I") || device.equals("i")) {
	        		...
	        		if(!UtilityObject.isStrEmpty(projectName) && 
	        				!UtilityObject.isStrEmpty(projectFolder) && 
	        				!UtilityObject.isStrEmpty(projectScheme) && 
	        				!UtilityObject.isStrEmpty(projectTarget)) {
    		...
    		if(device.equals("I") || device.equals("i")) {
    			...
    			ArrayList<File> fileList = UtilityObject.getFileByExtensionType(sourcePath, ".ipa");
    			...
    			if(!fileList.isEmpty()) {
	    			...
	    			logger.warn("do webService + " + mitakeServiceUrl + File.separatorChar +"uploadData.do");
	    			
	    			int statusCode = httpService.send(mitakeServiceUrl + File.separatorChar +"uploadData.do", builder.build());
